...
class ChannelTest extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "instantiate d2d channel" in {
    test(new D2DSidebandChannel(BigInt(1), new SidebandParams(), fdiParams)) {
      c =>
        c.clock.step()
        println("welp")
    }
  ...
  it should "instantiate phy channel" in {
    test(new PHYSidebandChannel(BigInt(2), new SidebandParams(), fdiParams)) {
      c =>
        c.clock.step()
        println("welp")
    }
  ...
  it should "send something for you" in {
    test(new channel_wrapper()) { c =>
      ...
      for (i <- 0 until (c.sbParams.sbNodeMsgWidth / c.fdiParams.sbWidth)) {
        c.io.channel.to_upper_layer.rx.bits.poke(
          packet((i + 1) * c.fdiParams.sbWidth - 1, i * c.fdiParams.sbWidth),
        )
        println(
          "subpacket: " + packet(
            (i + 1) * c.fdiParams.sbWidth - 1,
            i * c.fdiParams.sbWidth,
          ).litValue,
        )
        c.clock.step()
      }
      ...
      while (
        !c.io.channel.inner.node_to_layer_above.valid.peek().litToBoolean
      ) {
        c.clock.step()
      }
  ...
  it should "send something not for you" in {
    test(new channel_wrapper()) { c =>
      ...
      for (i <- 0 until (c.sbParams.sbNodeMsgWidth / c.fdiParams.sbWidth)) {
        c.io.channel.to_lower_layer.tx.bits.expect(
          packet((i + 1) * c.fdiParams.sbWidth - 1, i * c.fdiParams.sbWidth),
        )
        println(
          "subpacket: " + packet(
...
class channel_pair_wrapper extends Module {
  ...
  val dietodie_channel = Module(
    new D2DSidebandChannel(BigInt(1), sbParams, fdiParams),
  )
  val phy_channel = Module(
    new PHYSidebandChannel(BigInt(2), sbParams, fdiParams),
  )
  val d = Module(new channelmsgfactory())

  io.to_D2D <> dietodie_channel.io.inner
  io.to_PHY <> phy_channel.io.inner.switcherBundle
  phy_channel.io.inner.rawInput.noenq()
  phy_channel.io.inner.rxMode := RXTXMode.PACKET
  phy_channel.io.inner.inputMode := RXTXMode.PACKET
...
class channelmsgfactory extends Module {
  ...
  io.dtop := SBMessage_factory(
    SBM.LINK_MGMT_ADAPTER0_REQ_DISABLE,
    src = "D2D",
    remote = false,
    dst = "PHY",
  )
  io.ptod := SBMessage_factory(
    SBM.MBINIT_REVERSALMB_CLEAR_ERROR_REQ,
    src = "PHY",
    remote = false,
    dst = "D2D",
  )
  io.dtop_cmp := SBMessage_factory(
    SBM.COMP_0,
    src = "D2D",
    remote = false,
    dst = "PHY",
