...
class StallHandlerTest extends AnyFlatSpec with ChiselScalatestTester {
    ...
    it should "complete stall handshake" in {
        test(new RDIStallHandler()) { c => 
            //init
            for( a <- 0 until 5){
                c.io.rdi_pl_stallreq.poke(false.B)
                c.io.mainband_stalldone.poke(false.B)
                c.clock.step(1)
                c.clock.step(1)
                ...
                // mainband reply
                c.io.mainband_stalldone.poke(true.B)
                c.clock.step(1)
                ...
                for(i <- 0 until 10){
                    ...
                    c.io.mainband_stalldone.expect(true.B)
                    c.clock.step(1)
                }
                // falling edge of req
                c.io.rdi_pl_stallreq.poke(false.B)         
    ...
    it should "complete stall handshake" in {
        test(new FDIStallHandler()) { c => 
            for( a <- 0 until 5){
                //init
                c.io.linkmgmt_stallreq.poke(false.B)
                c.clock.step(1)
                // check if the initial signals are all false
                for(i <- 0 until 10){
                    c.io.linkmgmt_stalldone.expect(false.B)
                    c.io.fdi_pl_stallreq.expect(false.B)
                    c.clock.step(1)
                }
                // link management require handshake
                c.io.linkmgmt_stallreq.poke(true.B)
                c.clock.step(1)
                ...
                while(c.io.linkmgmt_stalldone.peek().litValue != true.B.litValue){
                    c.io.fdi_pl_stallreq.expect(false.B)
                    c.clock.step(1)
                }
