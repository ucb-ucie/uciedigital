...
case class SidebandParams (
    val NC_width: Int = 32,
    val MSG_width: Int = 128,
    val CDT_max: Int = 32
)

class SidebandLinkNode (val params: SidebandParams) extends Module {
    val io = IO(new SidebandLinkIO(params))

    val tx_ser = Module(new SidebandLinkSerializer(params))
    val rx_des = Module(new SidebandLinkDeserializer(params))
    val rx_queue = Module(new SidebandPriorityQueue(params))
...
class SidebandNode (val params: SidebandParams) extends Module {
    val io = IO(new SidebandNodeIO(params))
    // as a healthy brocolli
    val w = params.MSG_width
    val tx_ser = Module(new SidebandSerializer(params))
    val rx_queue = Module(new SidebandPriorityQueue(params))
    val rx_des = Module(new SidebandDeserializer(params))

    // Connect outer signals
    io.outer.tx <> tx_ser.io.out
    io.outer.rx.valid <> rx_des.io.in.valid
    io.outer.rx.bits <> rx_des.io.in.bits
    io.outer.rx.credit := rx_queue.io.deq.fire && (!SBM.isComplete(rx_queue.io.deq.bits.asUInt))
...
class SidebandPriorityQueue (val params: SidebandParams) extends Module {
    val w = params.MSG_width
    ...
    val p0_queue = Module(new Queue(UInt(w.W), 4)) // highest priority queue, for access completion packets
    val p1_queue = Module(new Queue(UInt(w.W), params.CDT_max)) // second highest priority queue, for message packets
    val p2_queue = Module(new Queue(UInt(w.W), params.CDT_max)) // lowest priority queue, for acess request packets

    val enq_arb = Module(new SidebandEnqArbiter(params))
    val deq_arb = Module(new SidebandDeqArbiter(params))
...
class SidebandEnqArbiter (val params: SidebandParams) extends Module {
    val w = params.MSG_width
    val io = IO(new Bundle {
        val out = Vec(3, Decoupled(UInt(w.W)))
...
class SidebandDeqArbiter (val params: SidebandParams) extends Module {
    val w = params.MSG_width
    val io = IO(new Bundle {
        val out = Decoupled(UInt(w.W))
        val in = Vec(3, Flipped(Decoupled(UInt(w.W))))
    })
    val lIndex: Array[Int] = Array(0, 1, 2)
    io.out.valid := io.in(0).valid || io.in(1).valid || io.in(2).valid
    // traffic arbitration
    when (io.in(0).valid) {
        lIndex.foreach(i => io.in(i).ready := (if (i == 0) io.out.ready else false.B))
        io.out.bits := io.in(0).bits
    } .elsewhen (io.in(1).valid) {
        lIndex.foreach(i => io.in(i).ready := (if (i == 1) io.out.ready else false.B))
        io.out.bits := io.in(1).bits
    } .elsewhen (io.in(2).valid) {
        lIndex.foreach(i => io.in(i).ready := (if (i == 2) io.out.ready else false.B))
...
class SidebandSerializer (val params: SidebandParams) extends Module {
    val nc_w = params.NC_width
    val msg_w = params.MSG_width
    val cdt_max = params.CDT_max
    val io = IO(new Bundle {
        val in = Flipped(Decoupled(UInt(msg_w.W)))
        val out = new Bundle {
            val bits = Output(UInt(nc_w.W))
    ...
    val dataBits = msg_w
    val dataBeats = (dataBits - 1) / nc_w + 1
    ...
    io.in.ready := (!sending && (SBM.isComplete(io.in.bits.asUInt) || (current_credit>0.U))) // set ready if credit>0 or completion packet
    io.out.valid :=  (((current_credit>0.U) || isComplete) && sending) // Don't need to check credit for access completion packets
    io.out.bits := data(nc_w-1, 0)
    ...
    when (io.out.valid) { data := data >> nc_w.U }
...
class SidebandDeserializer (val params: SidebandParams) extends Module {
    val nc_w = params.NC_width
    val msg_w = params.MSG_width
    val io = IO(new Bundle {
        val in = new Bundle {
            val bits = Input(UInt(nc_w.W))
    ...
    val dataBits = msg_w
    val dataBeats = (dataBits - 1) / nc_w + 1
    val data = Reg(Vec(dataBeats, UInt(nc_w.W)))
...
class SidebandLinkSerializer (val params: SidebandParams) extends Module {
    ...
    io.in.ready := (waited) // set ready if credit>0 or completion packet
...
class SidebandLinkDeserializer (val params: SidebandParams) extends Module {
    val nc_w = params.NC_width
    val msg_w = params.MSG_width
    val io = IO(new Bundle {
        val in = new Bundle {
