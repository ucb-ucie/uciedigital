...
/**
  * LinkResetSubmodule handles the transition of FDI/RDI state machine
  * from Reset, Active, and  Retrain to LinkReset state. The 
...
class LinkResetSubmodule (params: D2DAdapterParams) extends Module {
    ...
    when(io.link_state === PhyState.reset ||
        ...
        }.elsewhen(io.fdi_lp_state_req === PhyStateReq.linkReset &&
        ...
        when(io.linkreset_sb_rcv.valid && 
        ...
        // Check whether there is inflight disaabled response sbmsg from partner link
        when(io.linkreset_sb_rcv.valid && 
        ...
        when(linkreset_fdi_req_reg && io.linkreset_sb_snd.ready &&
