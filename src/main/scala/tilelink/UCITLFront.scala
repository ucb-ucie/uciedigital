  ...
  */
  ...
  when(out.d.fire) {
    when (managerEdge.last(out.d)) { // wait for all the beats to arrive
      ...
      txDTLPayload.mask    := 0.U//out.d.bits.mask -> doesn't exist
  ...
  when(inward.io.deq.fire) {
    ...
    uciTxPayload.data(4) := inward.io.deq.bits.data(319,256)
...
trait CanHaveTLUCIAdapter { this: BaseSubsystem =>
  p(UCITLKey).map { params =>
    ...
    bus.coupleTo(s"ucie_tl_port") { uciTL.managerNode := TLWidthWidget(8) := _ } //manager node because SBUS is making request?
    bus.coupleFrom(s"ucie_tl_port") { _ := TLWidthWidget(64) := uciTL.clientNode }
  }
}
